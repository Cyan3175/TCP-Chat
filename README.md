...
**10.5 的变化**：**删掉了 10.3 / 10.4 的「液态玻璃」整套东西** —— 玻璃面板、彩色壁纸层、Win2D 折射后端、苹果式自绘开关与滑块，以及设置里的玻璃开关 / 质量滑杆 / 实现选择全部移除，界面回到 10.2 那种朴素的不透明面板；程序也不再依赖 Win2D。
...
1. 下载 `TCP-Chat-10.5.exe`，**放在哪个目录都行**，双击即可（自包含，无需安装 .NET 或 Windows App Runtime）
2. 首次启动会问昵称，填完即可开始聊
...
dotnet publish -c Release -o ..dist      # 发布: 只产出一个 TCP-Chat-10.5.exe
```
