...
> 本文是**总说明**：功能、用法、编译、测试、已知限制都在这里。每个版本的详细改动见 [Releases](https://github.com/Cyan3175/TCP-Chat/releases) 里的发布说明；文末有[版本历史](#版本历史)摘要。

* * *

## 快速开始

1. 下载 `TCP-Chat-11.5.2.exe`，**放在哪个目录都行**，双击即可（自包含，不需要装 .NET，也不需要 Windows App Runtime）
...
* **Markdown 渲染**：标题 `#`、**粗体**、*斜体*、***粗斜体***、~~删除线~~、`行内代码`、==高亮==、H~2~O、x^2^、围栏代码块（语言标注 + 关键字着色，支持洛谷的 `line-numbers` 行号与 `lines=6-9` 高亮）、有序/无序/嵌套列表、`- [x]` 任务列表、表格（`:---` / `:---:` / `---:` 对齐，支持 `^` / `<` 合并与 `::cute-table{tuack}`）、引用块、`> [!NOTE]` 提示块、分割线、链接与裸网址、`![图](网址)`（自动下载显示）、脚注 `[^1]`、定义列表、`:::容器`、`:smile:` emoji 短代码· 纯文本消息走**快路径**，不会被当成 markdown（`3 - 2 = 1`、`1.5 倍`、`a_b_c` 原样显示）；单条超过 2 万字符自动退回纯文本，保证滚动流畅
...
* **液态玻璃**：顶栏、输入栏、状态栏、每个消息气泡都是独立注册的玻璃面（Win2D 效果链：背景采样 → 高斯模糊 → 湍流噪声 → 边缘遮罩 → 位移折射 → 叠色 → 饱和/对比 → 圆角裁切 + 高光 + 1px 亮边），背景优先用系统桌面壁纸，读不到就程序生成渐变；质量滑块 0~100 即时生效（湍流层数 1~4、模糊 0.6×~1.15×、渲染分辨率上限 1.0×~4.0×）；关掉就回到普通主题外观· **实测**（窗口 1107×744、5 个玻璃面）：质量 10 **3.7 ms** / 质量 60 **4.3 ms** / 质量 100 **5.1 ms** 一帧；带 50 个气泡的极端情况质量 60 约 29 ms，且只在玻璃面位置变化时重画，滚出视口的面直接跳过· **玻璃下的可读性（11.4 修）**：气泡色调按当前消息实时取色（列表回收复用容器时不会把「自己/别人」判错），昵称/时间在玻璃模式下有跟主题走的衬底与专用字色· **稳定性**：玻璃相关的回调（绘制、量位置、气泡注册）全部包了异常保护 —— 画玻璃出问题最多这一帧不画，不会把窗口带崩；XAML 未处理异常会写进 `%LOCALAPPDATA%\TCPChat\crash.log` 并保持程序存活
...
| 密钥  | `PBKDF2-HMAC-SHA256(密码, 盐, 100000 次)` → 32 字节；盐 = `SHA256("TCPChat10/v2 |
...
需要 **.NET 10 SDK** + **Windows 11 SDK 10.0.26100**（或 VS 2022 Build Tools 带 UWP/WinUI 工作负载）。
    ...
    dotnet publish -c Release -o ..\dist1152   # 发布: 只产出一个 TCP-Chat-11.5.2.exe

关键工程设置：

* `WindowsPackageType=None`（免打包运行，双击 exe 即用）、`WindowsAppSDKSelfContained=true`（把 Windows App Runtime 打进去，目标机不需要预装运行时）
* `PublishSingleFile` + `IncludeAllContentForSelfExtract` + `EnableCompressionInSingleFile`（发布成**单个 exe**；代价是首次启动会把自己解压到 `%TEMP%\.net\TCP-Chat-11.5.2\` 下，之后复用）
...
支持的动作：`wait:N` / `waitmsg:N` / `send:文本` / `sendmd:文件路径`（把整份 markdown 当成一条消息发出去） / `sendq:引用|正文` /`font:字体名` / `fonts` / `crypto:!序号|密码A|密码B` / `theme:0|1|2` / `zoom:+|-|0|1.25` / `glass:on|off` / `glassq:0~100` / `glassinfo` / `glassshot:路径` /`mic` / `voice` / `attach:路径` / `voicefile:路径|毫秒` / `receivecheck` /`scroll` / `mscroll:0~1`（消息列表滚到指定比例，便于核对长消息） / `menu` / `hidemenu` / `settings` / `dshot:路径` / `dscroll:0~1` /`dfontdrop` / `closedlg` / `shot:路径` / `notify:文本` / `keytest` / `log:文本` / `quit` /`mdpreview:文件路径[|other]`（把本地 markdown 当成一条消息渲染出来，**不发送**，用于核对公式/洛谷语法排版）。
...
* **单文件 exe 首次启动稍慢**：会把自己解压到 `%TEMP%\.net\TCP-Chat-11.5.2\`（约 240 MB，之后复用；换新版本会再解一份），程序每次启动会自动清理自己以前留下的旧解压目录
