...
**10.3 的变化**：加入**苹果「液态玻璃」外观**（顶栏 / 输入区 / 状态栏 / 消息气泡变成浮在彩色壁纸上的半透明玻璃），设置里有一个**开关**和一个 **1–10 档的质量滑块**，可实时预览并记住。
...
1. 下载 `TCP-Chat-10.3.exe`，**放在哪个目录都行**，双击即可（自包含，无需安装 .NET 或 Windows App Runtime）
2. 首次启动会问昵称，填完即可开始聊
...
### 界面
- **苹果「液态玻璃」外观**（默认开）：顶栏、输入区、状态栏是三块浮在彩色渐变壁纸上的圆角玻璃面板，消息气泡也是半透明玻璃，边缘有一道从上往下淡出的高光；设置里可以**开关**，并用 **1–10 档质量滑块**调磨砂浓度（1 = 几乎全透明最清淡，10 = 厚磨砂），拖动时主窗口实时预览
...
| **玻璃质量** | 1–10 档磨砂浓度（约 45% ~ 95%）；关掉玻璃时滑杆变灰，但取值会记住 |
...
TCPChat10/
  ...
  Services/
    ...
    LiquidGlass.cs         液态玻璃的配色换算(磨砂浓度 / 高光)
...
- **离线（55 项，不需要网络）**：密钥派生一致性、加解密往返、密码/目录/文件名不一致时解不开、篡改密文被 GCM 拦下、上行 JSON 不含明文、设置文件里字体与加密密码的持久化读写、设置文件位置与旧默认目录迁移、**液态玻璃开关与质量档位（默认值 / 夹取 / 强度与磨砂浓度换算 / 持久化）**、系统字体列表（数量/去重/无竖排变体）、10.0 老消息兼容（附件字段被忽略、纯附件消息被识别为"无正文"）
...
- **液态玻璃是"画"出来的，不是真模糊**：Windows App SDK 2.x 只开放了 Composition 的 SceneLightingEffect，没有高斯模糊效果（`Microsoft.UI.Composition.Effects` 里能用的只有两个接口），拿不到"把面板后面的内容糊掉"的能力。好在背景是自己画的柔和色块壁纸，本来就是糊的，所以玻璃感用**半透明磨砂染色 + 上边缘高光 + 圆角悬浮**叠出来，质量滑块调磨砂浓度；纯画刷实现，不额外吃显卡，也没有常驻动画
