...
**10.5 的变化**：**回车键分工调整**（`Enter` 发送，`Shift+Enter` 与 `Ctrl+Enter` 都是换行）；**每收到一条新消息弹一个系统通知并让任务栏图标闪烁**；设置对话框改成**圆角**并跟随主题；修了浅色/深色下的文字可读性问题。**同时删掉了 10.3 / 10.4 的「液态玻璃」整套东西** —— 玻璃面板、彩色壁纸层、Win2D 折射后端、苹果式自绘开关与滑块，以及设置里的玻璃开关 / 质量滑杆 / 实现选择全部移除，界面回到 10.2 那种朴素的不透明面板；程序也不再依赖 Win2D。
