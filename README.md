...
- **语音消息**：点「🎤 语音」开始录，再点一下「停止」发送（`.wav`，气泡里显示时长与播放按钮，再点暂停）；麦克风要系统权限，拿不到会明确提示
...
- **离线（61 项，不需要网络）**：密钥派生一致性、加解密往返、密码/目录/文件名不一致时解不开、篡改密文被 GCM 拦下、上行 JSON 不含明文、**附件字节加解密往返 / 密码或文件名不对解不开 / 密文被改后解不开 / 附件元信息随密文走 / 文件名过滤 / 类型判定 / 大小与时长格式化**、设置文件位置与旧默认目录迁移、系统字体列表、10.0 老消息兼容、设置文件里字体与加密密码的持久化读写、设置文件位置与旧默认目录迁移、系统字体列表（数量/去重/无竖排变体）、10.0 老消息兼容（附件字段被忽略、纯附件消息被识别为"无正文"）
...
- **附件与语音**：整份文件会先读进内存再上传（几十 MB 没问题，超大文件请直接用资源管理器拷）；附件按文件名缓存在 `%LOCALAPPDATA%\TCPChat10\cache`；**设了加密密码时附件内容也是密文上传**（AES-256-GCM，消息文件名作为附加认证数据），密码不一致时对方只看到"解密失败"
- **视频缩略图**用 WinRT `MediaComposition` 取第一帧，取不到就退回文件卡片；**语音录制**需要系统麦克风权限（首次弹窗），没有麦克风或拒绝授权时会明确提示
